...
> 更新于 2026-09-07 · [English](./README_en.md)
...
| 🎮 [四色定理填色](./function/Game/FourColorGame.html) | 基于四色定理的地图填色解谜，相邻区域不可同色，14 关难度递增、带提示与计时 |
| 🔥 [火种纪元](./function/Game/HuoZhongJiYuan.html) | 分支式文明放置养成：研究解锁科技/仙侠/魔法等路线，晋升时代获得永久传承 |
...
- **分类 + 搜索**：调试 / 硬件 / 软件 / 文档 / 其他 / 导航 / 游戏七个分类，一个工具可以同时挂多个分类；导航分类下的外链工具还会按「电子 / 机械 / 工业 / UI」等子分类再次分组；记不清名字就搜关键词，名称、描述、分类都能命中
- **中英文界面**：右上角一键切换，连图表轴标签、提示语都会跟着换
- **浅色 / 深色主题**：随你喜欢，选择会被记住，跨页面不丢
- **图标规范**：首页工具卡片、页面标题等界面图标统一使用**内联 SVG**（多色扁平、随主题配色），不使用 emoji；页面正文也尽量少用 emoji、多以文字或内联图标代替，保持风格统一
- **收藏夹**：常用工具打星，分类栏「收藏」一键筛选，本地保存；每个分类 / 子分类标题右侧的 ☆ 可**整类一键收藏 / 取消收藏**，点一下就把整个分类放进收藏
- **自定义网站**：右下角 ⚙ →「自定义网站」页签，输入网址、名称、图标、选导航子分类即可把**你自己的网页**加进首页导航；名称留空自动用网址域名，图标留空用名称首字，也支持图片上传；已添加的条目点击即可回填编辑
- **自定义搜索引擎**：右下角 ⚙ →「设置」页签可自由添加 / 删除引擎（内置 10 个不可删），支持 `?q=` 与 `%s` 两种地址写法；添加后自动出现在搜索引擎下拉里，**主页搜索框与禅模式搜索栏的引擎下拉切换**同步生效，刷新不丢
- **自定义背景**：「主题」页签可上传最多 9 张图片 / 视频作为页面背景（图片、视频合计 9 个），点击缩略图切换、悬浮可单独删除；另有卡片透明度 / 模糊度、背景通透度等外观微调，**禅模式下背景与通透度同样生效**
- **界面模式（☯）**：右下角悬浮按钮在 **常规 ↔ 禅** 两种界面间切换。禅模式全屏只留居中的放大搜索栏与**干支时钟**（`AM 08:26:34 | 戊辰`），右上角还有**主题切换按钮**，可直接换深浅色
- **多端适配**：手机、平板、电脑都排版正常
...
右下角 ⚙ 悬浮按钮打开「页面设置」弹窗（**设置 / 主题 / 自定义网站** 三个页签），在「设置」页签的**配置备份**区提供**导出配置 / 导入配置**两个按钮，可一键备份或恢复整站配置（主页 + 全部工具页），旧版单个 JSON 备份也可兼容导入。
...
- **导出**：生成 `yu-toolbox-config-<时间戳>.zip` 压缩包，结构如下：
  ...
  - `site/` — 主页配置（主题、语言、搜索引擎 **及自定义引擎**、自定义网站、收藏、外观、分类折叠等）
...
```
Yu_ToolBox/
├── index.html                    # 主页：导航 / 分类 / 搜索 / 收藏 / 吉祥物彩蛋 / 广告位 / 页面设置弹窗 / 禅模式
...
│   │   ├── index.css             # 主页特有样式：Hero、工具卡片网格、分类按钮、搜索框、收藏星标、吉祥物、广告位、页面设置弹窗、禅模式、右下角悬浮按钮
